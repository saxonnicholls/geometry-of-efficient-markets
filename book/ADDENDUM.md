...
The MUP portfolio is self-financing: rebalancing from $\hat{b}^M_t$ to $\hat{b}^M_{t+1}$
...
The portfolio weight $b_i$ and its conjugate momentum $\pi_i = \partial_{\dot{b}_i}\mathcal{L}$
...
For the log-optimal portfolio: $b^*\cdot\nabla L_T|_{b^*} = L_T(b^*)$ (by homogeneity of
...
The OPE of two return operators $\mathcal{O}_i(t_1)\mathcal{O}_j(t_2)$ as $t_1\to t_2$:
...
and it equals $\phi_i = b^*_i(\mu_i-\bar\mu)$.**
...
| A33 | Market partition function = det$(\Delta_M)^{-1/2}$ | PATH\_INTEGRAL | Proved |

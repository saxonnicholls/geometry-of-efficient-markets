...
We prove that the stable rank $r_{\rm eff}$ of the Fisher matrix at $b^{\ast}$ controls the dimension
...
$b^{\ast} = \arg\max_{b \in \Delta_{d-1}} r(b)$ is the empirical log-optimal (Kelly) portfolio.
...
*The value of the action at the critical path is $S(b^{\ast}, 0) = \int_0^T r(b^{\ast}(s),s)\,ds = \log W_T(b^{\ast})$.*
...
$\int_\Delta e^{T \cdot r(b)/\varepsilon^2}\,d\mu(b)$ is dominated by the saddle point at $b^{\ast}$.*
...
where $F(b^{\ast}) = -\nabla^2_b r(b^{\ast}) = \frac{1}{T}\sum_t \frac{x_t x_t^T}{(b^{*T}x_t)^2}$
...
$t \to \infty$, $\pi_t \to \delta_{b^{\ast}}$ — the posterior concentrates on the log-optimal
...
converges to $\delta_{b^{\ast}}$ at rate $e^{-\lambda_1 t}$.*
...
*where $r_{\rm eff}(F) = \|F\|_F^2/\|F\|_2^2$ is the stable rank of the Fisher matrix at $b^{\ast}$.*
...
**Proof sketch.** The stable rank of $F(b^{\ast})$ controls how many spherical harmonics $Y_{k,l}$
...
a hidden Markov model), the log-optimal portfolio $b^{\ast}(t)$ is a stochastic process on $\Delta_{d-1}$.
...
that the universal portfolio wealth remains a FK functional for the joint process $(B_t^\varepsilon, b^{\ast}(t))$ on $\Delta_{d-1} \times \Delta_{d-1}$, and that the stochastic Stokes theorem (6.10)
...
equals the **regret** $\log W_T(b^{\ast}) - \log S_T^{\ast}$ of the universal portfolio relative to the
...
the first-order condition $\nabla_\Delta r(b^{\ast}) = \lambda \mathbf{1}$ (gradient parallel to the
...
The **Newton step** in Fisher–Rao geometry for updating $b^{\ast}$ after observing a new return $x_t$ is:

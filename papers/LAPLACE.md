...
Now $C(b^{\ast})_{ij} = b^{\ast}_i(\delta_{ij} - b^{\ast}_j)$ and $F(b^{\ast};\tau)_{ij}$ are both symmetric matrices.
...
$\frac{1}{6}\sum_{j,k,l}\kappa_{jkl}\,\mathbb{E}_G[v_iv_jv_kv_l]$ where the fourth Gaussian

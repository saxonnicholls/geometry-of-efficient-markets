...
the Chern-Simons partition function on $S^3$. Since we established in FIBER_BUNDLES
...
(from MINIMAL_SURFACE Section 8.1), which in $S^3_+$ is a simple arc from one
...
For the trefoil: $\Delta_{3_1}(t) = t - 1 + t^{-1}$. Is this of the form $ff^{\ast}$?
...
portfolio time series $b^{\ast}(t)$, project it into $S^3_+$, compute the winding numbers,

...
**Abstract.**  
The portfolio simplex $\Delta_{d-1}$ equipped with the Fisher-Rao metric IS a
...
is minimised: $c_{\rm LZ}^{\rm Hilbert} \leq c_{\rm LZ}^{\rm arbitrary}$ always.
...
affecting stock $i$ moves the portfolio from $b^*$ to $b^* + \delta e_i$ in the
simplex. The perturbation propagates through the Delaunay graph of the Voronoi
partition at speed governed by the Jacobi spectral gap $\lambda_1$. At time $t$
...
the heat kernel of the manifold Laplacian $e^{t\Delta_M}$, which at short times is
...
structure. The systemic risk measure corresponds to the Cheeger constant $h_M$ of
the manifold: a market with small $h_M$ (a manifold with a thin bottleneck) has
...
The portfolio simplex $\Delta_{d-1}$ equipped with $g^{\mathrm{FR}}$ is a Riemannian
manifold of dimension $d-1$. The market manifold $M^r \subset \Delta_{d-1}$ is an
...
**For $d=3$ (three-asset market):** $u \in S^2_+$ (positive octant of the 2-sphere).
The portfolio is literally a point on the surface of a sphere — geospatial indexing
on $S^2$ applies without modification.

**For $d=4$ (four-asset, Clifford torus market):** $u \in S^3_+$ (positive orthant
...
**The H3 market index.** For a three-asset portfolio $b = (b_1,b_2,b_3)$ mapped
to $u = (\sqrt{b_1},\sqrt{b_2},\sqrt{b_3}) \in S^2_+$:
...
where $C_\ell \approx \pi/(2\cdot 7^{\ell/2})$ is the cell radius at resolution $\ell$.
...
H3 prefix of length $\ell$. This is equivalent to being within Fisher-Rao distance $C_\ell$.

**Theorem 2.1** *(H3 locality)*. *For two portfolios $b_1$, $b_2$:*
...
$d_{g^{\mathrm{FR}}}(b_1,b_2) \approx C_{\mathrm{LCP}(b_1,b_2)}$ — the distance
...
| Resolution $\ell$ | Cell radius $C_\ell$ | Market interpretation | Number of cells |
|:-----------------:|:--------------------:|:---------------------|:---------------:|
| 0 | $\pi/4$ | Full simplex (no structure) | 8 (on $S^2_+$) |
...
**The S2 market index** on $S^2_+$ uses only the 3 cube faces that intersect the
...
axes (as for the multi-CAPM, where $M = S^r_+$ and Voronoi cells are coordinate
...
The S2 hierarchy at level $\ell$ on portfolio $S^2_+$ partitions the market manifold
...
$(b_1 \gtrless b^*_1, b_2 \gtrless b^*_2)$ — the Clifford torus Voronoi cells.
...
has a remarkable property: the first $n$ bits $b_1\cdots b_n$ determine the depth-$n$
...
Fisher-Rao ball is elongated in directions of small portfolio weight $b_i$ and
...
**Portfolio nearest-neighbour search.** For a new market state $b_{\rm new}$, the
...
**The collision probability** for two portfolios $b_1, b_2$ is:
...
The **geohash** of a portfolio $b \in \Delta_{d-1}$ is:

$$\mathrm{GH}(b) = \mathrm{H3}(\sqrt{b}, \ell) \tag{6.3}$$

— the H3 cell ID of the Bhattacharyya image at resolution $\ell$. This is a compact
string (8–16 characters) that identifies the portfolio to precision $C_\ell$.

**Geohash proximity:** $\mathrm{GH}(b_1)$ and $\mathrm{GH}(b_2)$ share a prefix of
length $k$ iff $d_{g^{\mathrm{FR}}}(b_1,b_2) \lesssim C_k$. This is the portfolio
...
A shock to stock $i$ at time $t_0$ moves the log-optimal portfolio:
...
cell (if $\delta_i$ is large enough to cross a Voronoi boundary). The perturbation
...
with initial condition $\delta b(b,0) = \delta_i e_i\cdot\mathbf{1}_{b=b^*}$.
...
*where $p_t(b,b')$ is the heat kernel on $(M^r, g_M)$ from MARKET\_PROCESSES.*

*For the CAPM ($M=S^1_+$, Jacobi process): the contagion kernel is the Jacobi
polynomial series (MARKET\_PROCESSES equation 2.3).*

*For the Clifford torus ($M=T^2$): the contagion kernel is the theta function
$\vartheta_3$ (MARKET\_PROCESSES equation 4.4).*
...
where $w_{kl} = \pi_k\pi_l/d_{g^{\mathrm{FR}}}(v_k,v_l)$ (transition weight
...
**The spectral gap $\lambda_1^{\mathcal{D}}$** is the slowest contagion decay rate.
A market with small $\lambda_1^{\mathcal{D}}$ (thin bottleneck in the Delaunay graph)
...
Small Cheeger constant $h_M$ = narrow bottleneck in the market manifold = slow contagion
...
on manifold $M^r$ with Voronoi partition $\{A_k\}$:*
...
directly exposed via the contagion kernel (7.3) — a shock in $A_k$ propagates
to $A_l$ at rate $w_{kl}$ if $(A_k,A_l)$ are Delaunay adjacent.*
...
is the Delaunay edge weight $w_{kl}$ — which is the Fisher-Rao distance (how
...
$b^*_i(t)$ is in the crisis cell $A^{\rm crisis} = B_{g^{\mathrm{FR}}}(\partial\Delta, \varepsilon)$:
...
*(i) $\mathcal{SR}(M) \leq 2/\lambda_1(L_M)$ (from the Cheeger inequality).*
...
*(iv) The spectral gap $\lambda_1$ and the Cheeger constant $h_M$ both collapse to
...
of $h_M$ — a bottleneck minimises $|\partial S|/\min(|S|,|M\setminus S|)$. (iii) follows
...
(iv) At the neck pinch-off, $h_M\to 0$ and $\lambda_1\to 0$. $\square$
...
and $N_l(s)$ is the counting process of shocks to cell $l$.

**The natural excitation kernel** from the market geometry is the manifold heat kernel:

$$\phi_{kl}(t) = \alpha_{kl}\,p_t(v_k, v_l) \tag{9.2}$$

where $p_t(v_k,v_l)$ is the heat kernel evaluated at the Voronoi cell centres
($v_k, v_l$ are the centres) and $\alpha_{kl}$ scales the excitation strength.
...
iff the excitation strength exactly equals the spectral gap $\lambda_1$ of the manifold
Laplacian.*

*(iii) For the efficient market: $G(v_k,v_l) = $ (constant at the spectral gap)
...
Voronoi cell $A_k$ to $A_l$ at time $t$ is:*

$$I_{kl}(t) = w_{kl}\!\left|p_{b^*}(A_l|A_k) - \pi_l\right| \tag{10.1}$$

*where $p_{b^*}(A_l|A_k)$ is the transition probability from $A_k$ to $A_l$
and $\pi_l$ is the stationary probability of cell $A_l$.
This measures how much the market's current state in cell $A_k$ changes our
estimate of the probability of next being in cell $A_l$.*

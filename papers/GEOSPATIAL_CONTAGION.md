...
affecting stock $i$ moves the portfolio from $b^{\ast}$ to $b^{\ast} + \delta e_i$ in the
...
with initial condition $\delta b(b,0) = \delta_i e_i\cdot\mathbf{1}_{b=b^{\ast}}$.
...
polynomial series (MARKET_PROCESSES equation 2.3).*
...
*where $p_{b^{\ast}}(A_l|A_k)$ is the transition probability from $A_k$ to $A_l$

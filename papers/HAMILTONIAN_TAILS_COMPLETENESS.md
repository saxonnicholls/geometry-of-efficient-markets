...
| Ground state $\psi_0 \propto W_T^{1/2}$ | Saddle point $b^* = \mathrm{argmax}\,L_T$ |
| Energy gap $\lambda_1/T$ | Jacobi stability $\lambda_1$ |
| Tunnel splitting | Clifford torus instability |
| $\langle b\rangle_{\psi_0} = \hat{b}_T$ | $\arg\max W_T = b^*$ |
...
For a portfolio concentrated near $b^* = e_i$ (100% in asset $i$):
...
with $b^*_i$ fixed, $\alpha_i \to \infty$ (tails become Gaussian in the large-data limit);
...
Fisher–Rao volume factor $\prod_i b_i^{-1/2}$. Near $b^* = (1/d,\ldots,1/d)$ (uniform),
...
This is a Beta$(Tb^*_i - 1/2,\, T(1-b^*_i))$ distribution. The tail at $b_i \to 0$:
$\pi_T(b_i) \sim b_i^{Tb^*_i - 3/2}$, which in terms of the return $x = b_i/b^*_i$
...
$x^* = \sigma_I\cdot(H\mathrm{Area}(M))^{-1/2}$.*
...
$\alpha_{\rm eff} \approx 2$–$3$ in the deep tail.
...
*The market is complete ($|\mathcal{Q}| = 1$, unique EMM) iff $\dim N_{b^*}M = 0$,
...
$\theta \in T_{b^*}\Delta_{d-1}$ (the market price of risk vector) satisfying the
no-arbitrage condition (DERIVATIVES\_CONVEXITY equation 3.6):

$$\theta = F(b^*)^{-1}(\mathbb{E}[x] - r_f\mathbf{1}) \tag{3.2}$$

For the systematic (factor) component: $\theta_{\rm sys} = \Pi_{T_{b^*}M}\theta$
is uniquely determined by the factor loadings and the expected returns. For the
idiosyncratic (normal) component: $\theta_{\rm idio} = \Pi_{N_{b^*}M}\theta$ is
*not* determined by no-arbitrage — any value of $\theta_{\rm idio} \in N_{b^*}M$
gives a valid EMM. Hence the space of EMMs is parameterised by $N_{b^*}M$. $\square$
...
$N_{b^*}M$ under the Fisher–Rao metric — the space of unit-norm unhedgeable risk directions.*
...
from the bending of $M$ — even if the gradient lies in $T_{b^*}M$, the second derivative
...
on the index $I = b^{*T}S$, which is a function on $T_{b^*}M$ (the tangential direction
...
*(ii) Incompleteness: the space of EMMs has dimension $\dim N_{b^*}M = d-1-r$.*

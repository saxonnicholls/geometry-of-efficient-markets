...
From the Wright-Fisher diffusion on $\Delta_{d-1}$ restricted to $M^r$ (FK_SIMPLEX.md):

$$db^{\ast}(t) = -\varepsilon^2\vec{H}(b^{\ast}(t))\,dt + \varepsilon\,dW_M(t) \tag{1.1}$$

where $dW_M(t)$ is the Brownian motion on $(M, g_M)$ and $\vec{H}(b^{\ast})$ is the
mean curvature vector of $M$ at $b^{\ast}$.

**The three martingale regimes:**

- **Efficient market ($H \equiv 0$):** $db^{\ast} = \varepsilon\,dW_M$ — the portfolio
...
(HAMILTONIAN_TAILS_COMPLETENESS, Theorem 3.1). $\square$
...
From HAMILTONIAN_TAILS_COMPLETENESS (Theorem 3.1):
...
stopping problem on the market manifold with gain $G(b) = L_T(b) - L_T(b^{\ast})$
...
*Proof.* The gain function $G = L_T(b) - L_T(b^{\ast})$ satisfies $G(b^{\ast}) = 0$ (zero excess
...
they can enter a curvature-exploitation strategy (MINIMAL_SURFACE Theorem 9.1)
...
of the Kelly growth process $\{G_t = L_T(b^{\ast}(t))\}$ is the Feynman-Kac value function:*

$$S_t = u(b^{\ast}(t), t) = \mathbb{E}^{b^{\ast}(t)}\!\left[\int_t^T L_s\,ds\right] \tag{6.2}$$

*where $u$ solves the FK PDE from FK_SIMPLEX.md.*

*Proof.* $S_t$ is the smallest supermartingale dominating $G_t = L_T(b^{\ast}(t))$.
...
- **Compensator $A_t$:** The predictable increasing process capturing the drift. For the inefficient market: $dA_t = \varepsilon^2 H^2(b^{\ast}(t))\,dt$ — the compensator IS the squared mean curvature accumulated over time.
...
perturbation direction $u \in T_{b^{\ast}}M$ (staying on the manifold) or
$u \in N_{b^{\ast}}M$ (moving off the manifold).
...
direction — the curvature exploitation strategy of MINIMAL_SURFACE). Substituting:
...
The optimal control is $u^{\ast} = -\vec{H}$ (MINIMAL_SURFACE.md) and the HJB solution
...
(Conjecture 3.1 of MINIMAL_SURFACE.md):
...
gives $u^{\ast} = F(b^{\ast})^{-1}p_t = -\vec{H}(b^{\ast}(t))$ — **the Pontryagin maximum principle
...
The Doob-Meyer compensator $A_t = \varepsilon^2\int_0^t H^2\,ds$ grows at the same rate as the KL divergence between the inefficient and efficient Fokker-Planck stationary distributions (FOKKER_PLANCK_CFD.md). This unifies the martingale decomposition with the information-theoretic efficiency measurement: the predictable drift accumulated by an inefficient market equals the information-theoretic distance from efficiency, both controlled by $\|H\|^2_{L^2}$.

Additionally, $({\rm Sharpe}^{\ast})^2 = (Y_0^{\rm ineff} - Y_0^{\rm eff})/(\varepsilon^2 T)$ — the Sharpe ratio squared is the BSDE value premium per unit time and diffusion strength (new result R72). This connects the BSDE framework directly to the curvature characterisation of the Sharpe ratio in MINIMAL_SURFACE.md, giving a second independent derivation of ${\rm Sharpe}^{\ast} = \|H\|_{L^2}$ from backward stochastic calculus rather than differential geometry.

...
$\mathrm{Sh}_{\rm stab}(\Sigma) = |\lambda_1(L)|$ (the smallest eigenvalue magnitude
...
differential geometry on the portfolio simplex $(\Delta_{d-1}, g^{\mathrm{FR}})$ equipped
with the Fisher–Rao metric. The key objects are:

- The **market manifold** $\Sigma \subset \Delta_{d-1}$: the set of possible log-optimal
  portfolios under the market's factor structure, an $r$-dimensional submanifold for
  a market with $r$ systematic factors.
- The **Willmore inefficiency** $\mathcal{W}(\Sigma) = \int_\Sigma |H|^2\,d\mathrm{vol}$:
...
2. The rate at which Sharpe opportunities grow when the market is displaced from $\Sigma$:
   $\mathrm{Sharpe}^*(\Sigma_\varepsilon) = \varepsilon|\lambda_1(L)| + O(\varepsilon^2)$.
...
| Lawson surfaces $\tau_{m,n}$: index grows with genus | §6.4 | Proved |
...
Here $\Delta_\Sigma$ is the Laplace–Beltrami operator of $(\Sigma, g_\Sigma)$, $|II|^2 =
\sum_{a,b}|II(e_a,e_b)|^2$ is the squared norm of the second fundamental form (summed over
...
For a perturbation $\Sigma_\varepsilon$ of a minimal surface $\Sigma$ by $\varepsilon f\,\vec{\nu}$,
the mean curvature to first order is $H(\Sigma_\varepsilon) = -\varepsilon Lf + O(\varepsilon^2)$
...
($\mathrm{ind} = 0$): $\mathrm{Sh}_{\rm stab} = \lambda_1(L) > 0$ — perturbations in the
...
|\lambda_1(L)| > 0$ — perturbations in the direction of the first (most negative)
eigenvector $f_1$ of $L$ create Sharpe growing at rate $|\lambda_1|$; MCF drives the
market away from $\Sigma$ in this direction.*

*(iii) The eigenvector $f_1$ of $L$ corresponding to $\lambda_1$ is the **natural direction
...
perturbed. A high stability Sharpe (large $\lambda_1(L)$) means small perturbations create
...
rate $|\lambda_1|$, the Sharpe opportunity grows initially at rate $\varepsilon|\lambda_1|$
...
For a **stable** efficient market ($\lambda_1 > 0$), a perturbation $\varepsilon f_1$
...
For $d = 50$: $t_{1/2} = \frac{4\log 2}{48} \approx 0.058$ arbitrage cycles. Very fast.
...
*(i) Great $r$-sphere sections: $M = S^r_+ \subset S^{d-1}_+$ (any totally geodesic
...
all Lawson surfaces $\tau_{m,n}$ with $(m,n) \neq (1,0)$, and all higher-genus compact
...
$S^4_+$ are known); partial for $d = 6$ (some cases unknown in codimension $\geq 3$).

### 5.1 $d = 3$: Three assets

The ambient sphere is $S^2_+$ (positive octant of $S^2$). Minimal 1-submanifolds are
geodesic arcs; minimal 0-submanifolds are points.

| $r$ | Manifold | Topology | $|II|^2$ | $\lambda_1(L)$ | Index | Stable? |
|:---:|:---------|:--------:|:--------:|:--------------:|:-----:|:-------:|
...
The ambient sphere is $S^3_+$. Minimal submanifolds are well-studied.

| $r$ | Manifold | Topology | $|II|^2$ | $\lambda_1(L)$ | Index | Stable? |
|:---:|:---------|:--------:|:--------:|:--------------:|:-----:|:-------:|
| 1 | Great circle arc | $S^1$ | 0 | $+\frac{1}{2}$ | 0 | **Yes** |
| 1 | Great 2-sphere | $S^2$ | 0 | $+\frac{2}{4} = +\frac{1}{2}$ | 0 | **Yes** |
| 2 | Great 2-sphere $S^2_+ \subset S^3_+$ | $S^2$ | 0 | $+\frac{2}{4} = +\frac{1}{2}$ | 0 | **Yes** |
| 2 | Clifford torus $\tau_{1,1}$ | $T^2$ | $2$ | $-\frac{3}{2}$ | 5 | **No** |
| 2 | Lawson $\tau_{2,1}$ | Genus 2 | $>2$ | $<-\frac{3}{2}$ | $>5$ | **No** |
| 2 | Lawson $\tau_{m,n}$ ($mn \geq 2$) | Genus $mn$ | grows | $\to -\infty$ | grows | **No** |
...
Wait — let me be careful about signs. The Jacobi operator has $Lf = \Delta_\Sigma f +
...
Eigenvalues of $-\Delta_{T^2}$ on the Clifford torus (a flat square torus of side $\pi/\sqrt{2}$
restricted to the positive quadrant, side $\pi/(2\sqrt{2})$): the eigenvalues are
$\mu_{mn} = 2(m^2 + n^2)$ for $(m,n) \in \mathbb{Z}^2_{\geq 0}$.
...
Stability Sharpe: $|\nu_{00}| = 5/2$, so $\mathrm{Sh}_{\rm stab}(\text{Clifford}) = \frac{5}{2}\varepsilon$
...
The ambient sphere is $S^4_+$. This is the first dimension where an exceptional stable
case appears.

| $r$ | Manifold | Topology | $|II|^2$ | $\lambda_1(J)$ | Index | Stable? |
|:---:|:---------|:--------:|:--------:|:--------------:|:-----:|:-------:|
...
| 2 | Clifford-type $\tau_{1,1} \subset S^4$ | $T^2$ | $>0$ | $<0$ | $\geq 1$ | **No** |
| 2 | Lawson surfaces $\tau_{m,n} \subset S^4$ | Genus $mn$ | grows | $<0$ | grows | **No** |
...
First eigenvalue of $-\Delta_{\mathbb{R}P^2}$ restricted to even functions: $\mu_0 = 0$
(constants), $\mu_1 = 6$ (the degree-2 harmonics on $\mathbb{R}P^2$).
...
of $K = 1$), all curvatures scale by $1/4$: $|II|^2_{\rm our} = \frac{2/3}{4} = \frac{1}{6}$,
...
First eigenvalue of $-\Delta_{\mathbb{R}P^2}$ (rescaled to Bhattacharyya metric with $K=1/4$):
$\mu_0 = 0$, $\mu_1 = \frac{6}{4} = \frac{3}{2}$.
...
The ambient sphere is $S^5_+$. Now $r$ can range from 1 to 4.
...
We now compute $\lambda_1(L)$ — equivalently $\lambda_1(J)$ — explicitly for each family.
...
**Stability:** $\mu_0 = -\frac{d-2}{4} < 0$. Wait — again a negative eigenvalue? 
...
For $k=1$: $\mu_1 = \frac{r - (d-2)}{4}$. This is negative when $r < d-2$ (codimension $\geq 2$)
and positive when $r > d-2$.

**For the physically relevant case $r \ll d$ (few factors, many assets):** $r \leq d/2$
implies $r < d-2$ for $d \geq 4$, so $\mu_1 < 0$. This again gives instability for the
non-conformal modes!

**Resolution — boundary conditions.** The great $r$-sphere in $S^{d-1}_+$ has a boundary
$\partial S^r_+ = \partial\Delta_r$ — the faces of the subsimplex where some factor weights
are zero. The Dirichlet boundary condition $f = 0$ on $\partial S^r_+$ (perturbations that
fix the boundary) eliminates the low-frequency modes that cause instability. With Dirichlet
conditions, the first eigenvalue of $-\Delta_{S^r}$ with $f = 0$ on $\partial S^r_+$ is:

$$\mu_1^{\rm Dir} = \frac{\pi^2/4}{(r-1)/4} + O(1) > \frac{d-2}{4} \tag{6.4}$$

for the relevant range, giving $\mu_1(J) > 0$ and stability. The portfolio interpretation:
...
For the Clifford torus $\tau_{1,1} \subset S^3$ (portfolio: balanced two-factor, 4 assets):
$|II|^2 = 2$, $\overline{\mathrm{Ric}} = \frac{2}{4} = \frac{1}{2}$ (using $d=4$, $K=1/4$).

$$Jf = -\Delta_{T^2}f - \frac{5}{2}f \tag{6.7}$$

on the flat torus. Eigenvalues of $-\Delta_{T^2}$: $\mu_{mn} = 2(m^2+n^2)$ (on the
full-sphere Clifford torus; quarter-torus with Dirichlet BC: $\mu_{mn}^{\rm Dir} = 2(m^2+n^2)$
for $m,n \geq 1$, $\mu_{10} = 2$, etc.).

$J$-eigenvalues: $\nu_{mn} = 2(m^2+n^2) - 5/2$. Negative for $m^2+n^2 < 5/4$.
On the full torus: $(m,n) \in \{(0,0),(1,0),(0,1),(-1,0),(0,-1)\}$ — 5 negative modes.

| Mode $(m,n)$ | $\nu_{mn}$ | $\mathrm{Sh}_{\rm stab}$ (this mode) | Arbitrage direction |
|:---:|:---:|:---:|:---|
...
where $|II|^2_{\rm eff}$ is the part of the second fundamental form relevant to non-conformal
...
(using the known value $\lambda_1 = 5/3$ from the standard-normalisation computation,
...
**Prediction:** The rate of increase of the stability index $\frac{d}{dt}\mathrm{ind}(\Sigma_t)$
...
contributes $|\lambda_k(J)|$ to the Sharpe budget. Summing over the $k$ negative modes
...
That is exactly **5 negative eigenvalues**, confirming $\mathrm{ind}(\tau_{1,1}) = 5$.

In Bhattacharyya normalisation ($K = 1/4$, scale all curvatures by $1/4$):
$|II|^2 \to 2/4 = 1/2$, $\Delta_{T^2} \to \Delta_{T^2}/4$, so all $J$-eigenvalues scale
by $1/4$: $\nu_{mn}^{\rm Bhat} = (m^2+n^2-2)/4$. The index remains 5 (same negative modes).
The stability Sharpe for the $(0,0)$ mode: $|\nu_{00}| = 2/4 = 1/2$ in Bhattacharyya units;

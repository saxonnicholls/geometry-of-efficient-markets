...
- The **Sharpe–curvature theorem**: $\mathrm{Sharpe}^{\ast}(\Sigma) = \|H\|_{L^2(\Sigma)}$
  (proved in MINIMAL_SURFACE), so a minimal market has maximum Sharpe zero.
...
log-optimal portfolio (conditional on Conjecture 3.1 of MINIMAL_SURFACE). But not all
...
with the convention that $\mathrm{ind}(\Sigma) = \\#\{\lambda_k(L) < 0\}$ (negative
eigenvalues of $L$ correspond to unstable directions). The second variation of area is
$\delta^2\mathrm{Area}[\Sigma][f,f] = -\int_\Sigma f\,Lf\,d\mathrm{vol}$, so $\Sigma$
...
exactly **5 positive eigenvalues**, giving $\mathrm{ind}(\tau_{1,1}) = 5$.

| Mode $(m,n)$ | $\lambda_{mn}(L)$ | Arbitrage direction |
|:---:|:---:|:---|
| $(0,0)$ | $+5/2$ | Uniform tilt — group imbalance |
| $(\pm 1,0)$ | $+1/2$ | Within-group-1 rebalancing |
| $(0,\pm 1)$ | $+1/2$ | Within-group-2 rebalancing |

Stability Sharpe: $|\lambda_{00}(L)| = 5/2$, so $\mathrm{Sh}_{\rm stab}(\text{Clifford}) = \frac{5}{2}\varepsilon$
...
| 2 | Clifford-type $\tau_{1,1} \subset S^4$ | $T^2$ | $>0$ | $\geq 1$ | **No** |
| 2 | Lawson surfaces $\tau_{m,n} \subset S^4$ | Genus $mn$ | grows | grows | **No** |
...
- $\overline{\mathrm{Ric}}(\vec{\nu},\vec{\nu}) = 3$ (ambient $S^4$)
- Eigenvalues of $-\Delta_{\mathbb{R}P^2}$: $k(k+1)$ for $k = 0, 2, 4, \ldots$ (even harmonics only)
- The Jacobi operator $Lf = \Delta_{\mathbb{R}P^2}f + (4/3 + 3)f$ has eigenvalues $\lambda_k(L) = -k(k+1) + 13/3$

The $k=0$ mode gives $\lambda_0(L) = +13/3 > 0$, which is a positive eigenvalue of $L$.
...
For $k = 2$: $\lambda_2(L) = -6 + 13/3 = -5/3 < 0$ in the *unrestricted* operator,
...
$\lambda_1(L) = \frac{(d-2) - r}{4} \leq 0$, and all higher eigenvalues are more negative.
...
$\lambda_1(L) = \frac{(d-2) - r}{4} > 0$, which is a positive eigenvalue of $L$ ---
...
With Dirichlet conditions, the first admissible eigenvalue of $-\Delta_{S^r_+}$ is
...
(which is of order 1), so the effective eigenvalue $\lambda_1^{\rm eff}(L) > 0$ and
...
confirming $\mathrm{ind}(\tau_{1,1}) = 5$.

| Mode $(m,n)$ | $\lambda_{mn}(L)$ | $\mathrm{Sh}_{\rm stab}$ (this mode) | Arbitrage direction |
|:---:|:---:|:---:|:---|
...
$L_V = \Delta_{\mathbb{R}P^2} + (|II|^2 + \overline{\mathrm{Ric}})$ has positive

...
it is a symmetric function of $\{|\kappa_i|\}$ and is preserved under conformal
...
the centroid direction $1/(2\sqrt{b^*})$ lies in $\mathrm{Im}(V_r)$; the Sharpe of any
strategy is bounded by the projection of $1/(2\sqrt{b^*})$ onto the null space of $V_r^T$
...
|:-------|:----|:---------------|:---------|
| Embedding differential $d\iota_x$ | $U\Sigma V^T$ | All $= 1$ (isometric) | Tangent frame |
| Shape operator $A_k : T_xM \to T_xM$ | $V\mathrm{diag}(\kappa_i)V^T$ | $|\kappa_1|,\ldots,|\kappa_r|$ | Principal curvatures |
| Second fundamental form $II$ | $U\Sigma_B V^T$ | $\sigma_1(B),\ldots$ | Full extrinsic curvature |
| Jacobi operator $L: C^\infty(M) \to C^\infty(M)$ | Spectral: $L = \sum \lambda_k e_k \otimes e_k$ | $|\lambda_k|$ | Stability |
| Data matrix $X \in \mathbb{R}^{T\times d}$ | $U\Sigma V^T$ | $\sigma_1 \geq \ldots \geq \sigma_d$ | Factor structure |
...
condition $H = 0$ locally at $b^*$ to order $O(\sigma_{r+1}^2/\sigma_r^2)$.
...
obstruction: truncation may change topology, and $\int_M K\,d\mathrm{vol}$ is a topological
invariant.
...
where $|\kappa_i|$ are the singular values of the shape operator. A minimal surface is stable
iff no singular value of $A$ exceeds the threshold $\sqrt{\lambda_1(-\Delta_M) - (d-2)/4}$.
...
2. $1/(2\sqrt{b^*}) \in \mathrm{Im}(V_r)$, where $X = U\Sigma V^T$
...
by how $V_x$ rotates (the Levi-Civita connection) and how $U_x \setminus V_x$ (the normal
...
**Proof sketch.** The Laplacian of the embedding satisfies $\Delta_M \iota = r\vec{H} - r\iota$
(for immersions in $S^{d-1}$, this is the sphere version of the harmonicity condition). If
$H = 0$, then $\Delta_M \iota = -r\iota$. Conversely, $\Delta_M\iota = -\lambda\iota$ forces
...
condition $\Delta_M \iota = -r\iota$ becomes:

$$L_M \mathbf{X} = -r\mathbf{X} \tag{2.3}$$

where $L_M$ is the discrete Laplace–Beltrami operator on the sampled manifold. The columns
of $\mathbf{X}$ are eigenvectors of $L_M$ with eigenvalue $-r$. This is **the definition
...
The top-$r$ SVD components span the eigenspace of $L_M$ corresponding to $-r$ —
...
$A: T_xM \to T_xM$ is symmetric and has SVD = eigendecomposition:

$$A = V \mathrm{diag}(\kappa_1, \ldots, \kappa_{d-2}) V^T \tag{3.1}$$

The singular values $|\kappa_1| \geq |\kappa_2| \geq \ldots \geq |\kappa_{d-2}|$ are the
...
|:-------|:--------|:---------------|
| Mean curvature | $H = \frac{1}{d-2}\sum_i \kappa_i$ | $\frac{1}{d-2}\mathrm{tr}(\Sigma_A^{\rm signed})$ |
| $\|II\|_F^2$ | $\sum_i \kappa_i^2$ | $\|\Sigma_A\|_F^2$ |
| Gaussian curvature | $K = \prod_i \kappa_i$ | $\det(\Sigma_A^{\rm signed})$ |
...
$|\kappa_i|$ can be anything; it is the cancellation of signs that is required.

### 3.2 What the SVD preserves — and what it does not

**Preserved by orthogonal change of basis (SVD rotation):**

The SVD replaces $A$ by $VAV^T = \mathrm{diag}(\kappa_i)$. This is an orthogonal similarity
transformation, which preserves:

- $\mathrm{tr}(A) = \sum_i \kappa_i$ (the mean curvature, up to factor) ✓
- $\|A\|_F^2 = \sum_i \kappa_i^2$ (the Willmore density) ✓
- $\det(A) = \prod_i \kappa_i$ (the Gaussian curvature, up to sign) ✓
...
For a minimal surface in 3D ($d=3$, $r=2$), $A$ has exactly two eigenvalues $\kappa_1, \kappa_2$
with $\kappa_1 + \kappa_2 = 0$. The SVD is:
...
is minimal iff its shape operator has equal singular values $\sigma_1(A) = \sigma_2(A)$ with
...
$\kappa_i$ by its reciprocal $1/\kappa_i$.
...
**Proof.** Since $A$ is trace-free, $\sum_i \kappa_i = 0$. For the pseudoinverse, we need
$\sum_i 1/\kappa_i = 0$. These are two separate conditions and are NOT generally equivalent.
...
For $r > 2$: the condition $\sum_i \kappa_i = 0$ does not generally imply $\sum_i 1/\kappa_i = 0$.
We need an additional symmetry assumption — either that the principal curvatures come in
$\pm$ pairs (as for the Clifford torus: $\kappa_1 = -\kappa_2 = \kappa, \kappa_3 = -\kappa_4 = \mu$)
...
*(iii) Minimal surfaces with $r$ even and skew-symmetric spectrum $\{\kappa_1,-\kappa_1,\ldots,\kappa_{r/2},-\kappa_{r/2}\}$.*

*For general higher-dimensional minimal surfaces, $A^+$ is trace-free iff $A$ has this paired structure.* $\square$

**The duality in pictures.** For the Clifford torus ($\kappa_1 = +1, \kappa_2 = -1$ in
...
For a perturbation of the Clifford torus (the tilted torus with $\kappa_1 = \kappa + \varepsilon,
\kappa_2 = -\kappa + \varepsilon$, $H = \varepsilon \neq 0$):
...
$\{1/\kappa_i\}$ in place of $\{\kappa_i\}$. This corresponds to:
...
**For the Clifford torus (self-dual):** The perfectly balanced two-factor market ($\kappa_1 = -\kappa_2$)
...
(for surfaces in $S^3$ with $K_{\rm ambient} = 1/4$). The Eckart–Young truncation affects
...
but changes $\det(A) = \kappa_1\kappa_2 = -\kappa^2$ (for a minimal surface). Therefore,
...
The Jacobi operator $L = \Delta_M + |II|^2 + \overline{\mathrm{Ric}}(\vec{\nu},\vec{\nu})$
...
where $\phi_k$ are the Jacobi eigenfunctions and $\lambda_k$ the Jacobi eigenvalues.
...
surface $M^r \subset S^{d-1}$ with shape operators $\{A_k\}_{k=1}^{d-1-r}$ (one per
...
*where $\lambda_1(-\Delta_M)$ is the first nonzero Laplacian eigenvalue on $M$ (the spectral
gap). Conversely, if all singular values of all $A_k$ satisfy:*
...
either all $|\kappa_i| = 0$ or $\sum |\kappa_i|^2 \geq \frac{d-2}{d-1}$. This is an
...
For the great $r$-sphere: $\lambda_1(-\Delta_{S^r}) = r$ (in Bhattacharyya normalisation),
$|II|^2 = 0$, so all $\sigma_i = 0 < \sqrt{r - (d-2)/4}$ — stable.

For the Clifford torus: $\lambda_1(-\Delta_{T^2}) = 1/2$ (for the flat quarter-torus in
Bhattacharyya normalisation), $|II|^2 = 1/2$, so $\sigma_1 = \sigma_2 = 1/\sqrt{2}$.
...
$\sigma_i < \sigma^*$, and unstable iff any $\sigma_i > \sigma^*$.*
...
**Portfolio interpretation:** The shape operator singular values $\{|\kappa_i|\}$ are
...
the factor dynamics on $M$). If any estimated $|\kappa_i| > \sigma^*$: the market manifold
is unstable, and the instability Sharpe is $|\kappa_i^2 - \sigma^{*2}|^{1/2}$ per unit
perturbation.
...
The top-$r$ eigenvalues $\lambda_1 \geq \ldots \geq \lambda_r$ correspond to the factor
directions (tangent to the market manifold $M$), and the remaining $\lambda_{r+1},\ldots,\lambda_{d-1}$
...
*where $V_r = [v_1|\cdots|v_r]$ are the top-$r$ eigenvectors of $F(b^*)$.*
...
*Proof.* From (7.2), $H=0$ iff $b^* \perp_{g^{\mathrm{FR}}} v_k$ for all $k > r$.
In the Fisher-Rao metric $g^{\mathrm{FR}}_{ij} = \delta_{ij}/b^*_i$, the inner product is
$\langle u, w\rangle_{g^{\mathrm{FR}}} = \sum_i u_i w_i / b^*_i$. So $b^* \perp v_k$ means
$\sum_i b^*_i v_{ki} / b^*_i = \sum_i v_{ki} = \mathbf{1}^T v_k = 0$, but more precisely the
condition is $\langle 1/(2\sqrt{b^*}), v_k\rangle_{g^{\mathrm{FR}}} = 0$, i.e.\ $1/(2\sqrt{b^*})$
...
*This is the weighted residual of the projection of $1/(2\sqrt{b^*})$ onto $\mathrm{Im}(V_r)^{\perp}$,
...
- The same principal curvature directions as $M$ (same factor loading matrix $V_r$)
...
and zeros in the normal direction. The dual CAPM has factor curvatures scaled by $1/\lambda_{\rm factor}$
...
9. Geodesic curvature $\kappa_g$ of curves on $M$: not preserved by normal perturbations (depends on intrinsic geometry of $M$, which changes).
...
representation on $S^3$ involves a pair of holomorphic maps $(f_1, f_2): \mathbb{C} \to
\mathbb{C}^2$ with $|f_1|^2 + |f_2|^2 = 1$ (the Hopf fibration). The factor loading
...
$r$-submanifold $M$ of $S^{d-1}$ with stable rank $r_{\rm eff}(A) = \|A\|_F^2/\|A\|_2^2$:*

$$1 \leq r_{\rm eff}(A) \leq r \tag{10.2}$$

*with $r_{\rm eff}(A) = 1$ for the Clifford torus and $r_{\rm eff}(A) = r$ for totally
...
**Proof.** $r_{\rm eff}(A) = \|A\|_F^2/\|A\|_2^2 \geq 1$ always (stable rank is at least 1
for nonzero $A$) and $\leq r$ (trace-free $r\times r$ matrices have stable rank at most $r$).
For equality: $r_{\rm eff} = 1$ iff all singular values are equal (Clifford), $r_{\rm eff} = r$
...
concentration" of the market manifold. A high stable rank ($r_{\rm eff}(A) \approx r$)
means curvature is uniformly distributed across factor directions — consistent with a nearly
totally geodesic (CAPM-like) market. A low stable rank ($r_{\rm eff}(A) \approx 1$) means
...
| Rank-$r$ truncation | Data matrix $X$ | Locally yes | No (topology) | Yes if $\sigma_{r+1}=0$ |
...
1. **Efficiency testing via SVD residuals:** Compute $\Pi_{V_{d-r}^T}(1/\sqrt{b^*})$. If
...
2. **Dual market construction:** The pseudoinverse market $M^+$ is efficient iff $M$ is,
   ...
   markets that are dual to each other (same $V_r$, inverted $\Lambda$) and trade the spread
   between their efficiency measures.
3. **Curvature concentration as a risk measure:** The stable rank $r_{\rm eff}(A)$ measures
...
4. **The SVD stability threshold (6.5):** If any idiosyncratic eigenvalue $\lambda_k$ exceeds

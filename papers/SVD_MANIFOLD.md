...
**Paper II.10** — *The Geometry of Efficient Markets*
...
the centroid direction $1/(2\sqrt{b^{\ast}})$ lies in $\mathrm{Im}(V_r)$; the Sharpe of any
strategy is bounded by the projection of $1/(2\sqrt{b^{\ast}})$ onto the null space of $V_r^T$
...
2. $1/(2\sqrt{b^{\ast}}) \in \mathrm{Im}(V_r)$, where $X = U\Sigma V^T$
...
For a perturbation of the Clifford torus (the tilted torus with $\kappa_1 = \kappa + \varepsilon, \kappa_2 = -\kappa + \varepsilon$, $H = \varepsilon \neq 0$):
...
$\sigma_i < \sigma^{\ast}$, and unstable iff any $\sigma_i > \sigma^{\ast}$.*
...
the factor dynamics on $M$). If any estimated $|\kappa_i| > \sigma^{\ast}$: the market manifold
...
*This is the weighted residual of the projection of $1/(2\sqrt{b^{\ast}})$ onto $\mathrm{Im}(V_r)^{\perp}$,

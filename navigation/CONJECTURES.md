...
$J_\Gamma(q)=1$ for generic $q$ $\iff$ market is topologically CAPM.
*Open step:* Identify the Chern-Simons level $k$ in the portfolio action.

**C3. Alexander Polynomial = Factor Rotation Speed (Grade A)**
Roots of $\Delta_\Gamma(t)$ = eigenvalues of factor rotation matrix after one
...
*Open step:* Identify $q_0=e^{2\pi i/(k+2)}$ is not at an exceptional easy value.
...
$c_1(NM)\in\mathbb{Z}$ computable quarterly from rolling covariance.
Expected to jump at structural breaks.

**C10. Topologically Protected Alpha Is Quantised (Grade B)**
When $c_1(NM)\neq0$: alpha $= c_1/T$ per cycle.

**C11. Market Phase Transitions = Chern Number Changes (Grade B)**
Major structural breaks = integer jumps in $c_1(NM)$ with spectral gap closure.
...
Voronoi tessellation of $\Delta_4$ with five factor vertices has Penrose-tiling
...
$C_{\rm McKean}>C_{\rm BS}$ for maturity $T>1$ year. Testable on LEAPS.
...
**C22. $E_8$ Symmetry in 9-Asset Markets (Grade C)**
The Voronoi tessellation of $\Delta_8$ may have $E_8$ Weyl group symmetry.
...
to the Cheeger constant $h_M\to0$ and MCF singularity formation.
...
of Dyson class correspondence (Theorem 1.1(ii) of RANDOM_MATRIX).
...
$\partial/\partial a_i \log S_r(a,b,\gamma) = \phi_i = b^{\ast}_i(\mu_i - \bar\mu)$.
...
STOCHASTIC_CONTROL_KALMAN to FILTRATIONS.

**C30. Percolation Threshold = Hawkes Criticality (Grade B) ★★**
$p_c$ on the Delaunay graph and $\rho(A_{\rm Hawkes})=1$ are the same condition.
Would connect GRASSBERGER_PERCOLATION_GENERATING to GEOSPATIAL_CONTAGION.
...
- C22 ($E_8$ markets) — exceptional mathematics inside 9-asset portfolios
